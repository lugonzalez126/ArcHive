...
from app.dependencies import get_db, get_current_user, get_current_instructor
...
def create(curriculum_id: uuid.UUID, data: ModuleCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_instructor)):
...
def update(module_id: uuid.UUID, data: ModuleUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_instructor)):
    return update_module(db=db, module_id=module_id, data=data, current_user=current_user)


@router.delete("/modules/{module_id}")
def delete(module_id: uuid.UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_instructor)):
...
def reorder(module_id: uuid.UUID, new_position: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_instructor)):
